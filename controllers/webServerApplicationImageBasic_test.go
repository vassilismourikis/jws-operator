		...
		It("WebServerApplicationImageBasicTest", func() {
			...
			if noskip {
				...
				if isopenshift {
					...
					Expect(webserverstests.WebServerSecureRouteTest(k8sClient, ctx, thetest, namespace, "secureroutetest", "jboss-webserver56-openjdk8-tomcat9-openshift-ubi8", "/health", os.Getenv("HOSTNAME"))).Should(Succeed()) //tests if the created pod is accessible via the tls route created by the operator
